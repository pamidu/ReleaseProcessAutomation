...
printf "${GREEN}#######################################################${NC}\n"
printf "\t ${BLUE}press 1 to final release shell\n"
printf "\t press 2 to final releas mobile shell\n"
printf "\t press 3 to final releas other folder\n"
...
if [ "$number" == "1" ]; then
	read -p "shell release selected ,please confirm (y/n) : " confirm 
	...
	if [ "$confirm" == "y" ]; then
		if [  -d "shell" ];then
			cd /var/www/html
			sudo find -name ".git*" -exec rm -r -f {} \;
			...
			read -p "please enter commit message : " message
			sudo git commit -m "$message"
			sudo git remote add origin https://$username:$password@github.com/duobuilduser/shell.git
			sudo git push -u origin master
